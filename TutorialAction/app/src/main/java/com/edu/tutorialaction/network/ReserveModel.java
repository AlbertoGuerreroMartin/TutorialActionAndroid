...
public enum ReserveModel {
    ...
    public Subscription completeReserve(Observer<Object> observer, Context context, int reserveID, int duration) {
        // If there's a request in background, subscribe to it
        if (request != null) {
            return request.subscribe(observer);
        }
        ...
        // Load reserves from network
        SharedPreferences sharedPreferences = PreferenceManager.getDefaultSharedPreferences(context);
        NetworkManager.INSTANCE.getClient().completeReserve(sharedPreferences.getString(API_KEY_SHARED_PREFERENCES_KEY, ""), reserveID, duration)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(request);
